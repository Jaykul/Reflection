...
function Import-ExtensionMethod {
    ...
    foreach($Method in Get-ExtensionMethod @PSBoundParameters) {
        Write-Verbose ("[{0}]::{1}({2})" -f $Method.DeclaringType, $Method.Name, $Method.GetParameters().Count)
        ...
        if($Method.IsGenericMethod) { 
            ...
            if($Parameters.Count -gt 1) {
                $Parameters = @($Parameters[-1]) + @($Parameters[0..($Parameters.Count-2)])
            }
            Write-Verbose ($Parameters -join ",")
            Write-Verbose ($Types -join ",")
            # NOTE: the magic regex is to pull the $paramName off the end of the [ParamType]


            $Script = $GenericInvoke -f $ParamBlock, $Method.DeclaringType, $Method.Name, ($Types -join ","), ($Parameters -join ",")
        }
